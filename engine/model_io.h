 ...
 *
 * THE FORMAT (see docs/reference/model-format.md for the full spec)
 * ---------------------------------------------------
 *   header, 16 bytes:
...
static uint32_t model__pad4(uint32_t n) { return (0u - n) & 3u; }
...
static int model_load(Model* m, const char* path) {
    ...
    /* Header decoding is portable; borrowed float data requires LE binary32. */
    const uint32_t endian_probe = 1;
    if (*(const unsigned char*)&endian_probe != 1 || sizeof(float) != 4 ||
        FLT_RADIX != 2 || FLT_MANT_DIG != 24 || FLT_MAX_EXP != 128) {
        fprintf(stderr, "model_load: requires little-endian IEEE binary32 floats\n");
        return 1;
    }
    ...
    if (ntensors == 0 || ntensors > MODEL_MAX_TENSORS) {
    ...
    /* ---- tensor records ---- */
    {
        ...
        for (uint32_t t = 0; t < ntensors; ++t) {
            ...
            if (name_len == 0 || name_len > MODEL_MAX_NAME || off + name_len > m->size) goto truncated;
            if (memchr(m->blob + off, '\0', name_len)) goto truncated;
            memcpy(mt->name, m->blob + off, name_len);
            mt->name[name_len] = '\0';
            for (uint32_t prev = 0; prev < t; ++prev) {
                if (strcmp(m->tensors[prev].name, mt->name) == 0) {
                    fprintf(stderr, "model_load: duplicate tensor '%s'\n", mt->name);
                    goto fail;
                }
            }
            ...
            if (mt->ndim == 0 || mt->ndim > MODEL_MAX_DIMS) {
            ...
            for (uint32_t d = 0; d < mt->ndim; ++d) {
                ...
                if (mt->dims[d] == 0 || mt->n_elem > UINT32_MAX / 4u / mt->dims[d]) {
                    fprintf(stderr, "model_load: invalid or overflowing tensor shape\n");
                    goto fail;
                }
                mt->n_elem *= mt->dims[d];
            }
            ...
            if (off > m->size || nbytes > m->size - off) goto truncated;
        ...
        if (off != m->size) {
            fprintf(stderr, "model_load: trailing bytes after tensors\n");
            goto fail;
        }
    }
...
static inline const MTensor* model_require(const Model* m, const char* name) {
...
static inline int model_has_shape(const MTensor* tensor, uint32_t ndim,
                                  const uint32_t* dims) {
    if (!tensor || tensor->ndim != ndim) return 0;
